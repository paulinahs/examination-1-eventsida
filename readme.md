**1. Vad innebär semantisk HTML och varför har ni använt det på er eventsida?**
Semantisk HTML innebär att man använder HTML-element som beskriver innehållets betydelse. Exempel är `<header>`, `<nav>`, `<section>` och `<footer>`.

På vår eventsida har vi använt semantiska element för att göra strukturen tydligare för både användare och webbläsare. Till exempel används `<header>` för hero-sektionen och `<nav>` för navigationsmenyn.

**3. Vad är den största skillnaden mellan Flexbox och CSS Grid, och när ska man använda vilket verktyg? Motivera utifrån hur ni fördelade dem på er sida.**


## 2. Hur fungerar arv i CSS? Ge ett exempel från er egen kod.
Arv i CSS innebär att vissa egenskaper automatiskt förs vidare från ett föräldraelement till dess barn.

Ett exempel från vår kod är:

body {
    font-family: "Montserrat", sans-serif;
}



## 3. Vad är den största skillnaden mellan Flexbox och CSS Grid, och när ska man använda vilket verktyg? Motivera utifrån hur ni fördelade dem på er sida.


 

## Ownership/Copyright:
Hero Photo by <a href="https://unsplash.com/@anthonydelanoix?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Anthony DELANOIX</a> on <a href="https://unsplash.com/photos/person-performing-heart-hand-gesture-hzgs56Ze49s?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
